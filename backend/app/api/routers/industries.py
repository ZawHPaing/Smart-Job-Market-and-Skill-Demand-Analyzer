...
async def dashboard_metrics(
    ...
    return IndustryDashboardMetrics(
        year=data["year"],
...
async def top_industries(
    ...
    if by == "salary":
        ...
        return IndustryTopResponse(year=year, by=by, limit=limit, industries=[IndustryCard(**r) for r in rows])
...
async def top_industries_trends(
    ...
    return IndustryTopTrendsResponse(
...
# ✅ FIXED: only ONE endpoint, and it returns dicts (no nested Pydantic instances)
...
async def composition_top_occupations(
    ...
    # IMPORTANT: pass dicts, NOT IndustryTopOccRow(...) objects
    return IndustryTopOccCompositionResponse(
        year=year,
        industries_limit=industries_limit,
        top_n_occ=top_n_occ,
        rows=data.get("rows", []),
...
async def top_jobs(
    ...
    return IndustryTopJobsResponse(
        naics=naics,
...
async def industry_summary(
    ...
    return IndustrySummaryResponse(
        naics=naics,
        naics_title=naics_title,
        year_from=min(year_from, year_to),
        year_to=max(year_from, year_to),
